...
export default async function Page({
	...
	const hasAccount = alumni.alumniAccount;
	...
	return (
		<div className="space-y-5">
			...
			<Card>
				...
				<CardContent className="px-5 pt-2 pb-4 grid grid-cols-2 gap-3 sms:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
					<AlumniData label="LRN" data={alumni.lrn} />
					<AlumniData label="Student ID" data={alumni.studentId} />
					<AlumniData label="Batch" data={alumni.graduationYear.toString()} />
					<AlumniData label="Education Level" data={alumni.educationLevel} />
					<AlumniData label="Strand" data={alumni.strand} />
				</CardContent>
			</Card>
		</div>
	);
